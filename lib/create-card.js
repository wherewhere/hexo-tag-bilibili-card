 ...
 */
function createElement(imageProxy, { vid, type, title, author, cover, duration, views, danmakus, comments, favorites, coins, likes }) {
    ...
    attributes.push(`comments="${comments || 0}"`);
    attributes.push(`favorites="${favorites || 0}"`);
    attributes.push(`coins="${coins || 0}"`);
    attributes.push(`likes="${likes || 0}"`);
    if (imageProxy) {
...
function createCardById(imageProxy, id, log = console) {
    const cvid = id.slice(0, 2).toUpperCase();
    switch (cvid) {
        case "CV":
            return getArticleMessage(id, log).then(x => createElement(imageProxy, x));
        case "BV":
        case "AV":
        default:
            return getVideoMessage(id, log).then(x => createElement(imageProxy, x));
    }
}
 ...
 * @param {{warn: function(string): void}} log
 */
function createCard(imageProxy, args, log = console) {
    const id = args[0];
    switch (args[1]) {
        case "video":
            return getVideoMessage(id, log).then(x => createElement(imageProxy, x));
        case "article":
            return getArticleMessage(id, log).then(x => createElement(imageProxy, x));
        default:
            return createCardById(imageProxy, id, log);
    }
}
