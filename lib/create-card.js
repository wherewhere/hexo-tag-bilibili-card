...
function createCard(imageProxy, { v_id, v_cover, v_time, v_title, v_playview, v_danmaku, v_upname }) {
    return `<bilibili-card vid="${v_id}" title="${v_title}" upper="${v_upname}" cover="${v_cover}" duration="${v_time}" views="${v_playview}" danmakus="${v_danmaku}" image-proxy="${imageProxy}"></bilibili-card>`
}
